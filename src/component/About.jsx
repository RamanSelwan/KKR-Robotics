import React from 'react';
import { motion } from 'framer-motion';

const About = () => {
  return (
    <div className="bg-gray-900 text-white py-10 w-full" id="about">
      {/* Our Story Section */}
      <motion.section 
        className="text-center py-12"
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true }}
        transition={{ duration: 0.6 }}
      >
        <h1 className="text-4xl font-bold mb-4">Our Story</h1>
        <p className="text-lg max-w-2xl mx-auto">
          Pioneering the future of robotics since 2010. We're dedicated to creating innovative
          solutions that transform industries and empower the next generation.
        </p>
      </motion.section>

      {/* Our Mission Section */}
      <section className="bg-white text-gray-800 py-12">
        <motion.div 
          className="text-center mb-12"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-3xl font-bold">Our Mission</h2>
          <p className="text-lg max-w-3xl mx-auto mt-4">
            To advance human potential through cutting-edge robotics and automation solutions
            that make the world more efficient, safer, and more connected.
          </p>
        </motion.div>

        <div className="flex flex-col md:flex-row justify-center gap-8">
          {[
            { icon: "💡", title: "Innovation", description: "Pushing boundaries in robotics technology through continuous research and development." },
            { icon: "🤝", title: "Collaboration", description: "Working together with industry partners to create meaningful solutions." },
            { icon: "🌍", title: "Impact", description: "Making a positive difference in industries and communities worldwide." }
          ].map((item, index) => (
            <motion.div 
              key={index} 
              className="text-center"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: index * 0.2 }}
            >
              <div className="text-5xl mb-4">{item.icon}</div>
              <h3 className="text-xl font-bold">{item.title}</h3>
              <p>{item.description}</p>
            </motion.div>
          ))}
        </div>
      </section>

      {/* Company Timeline Section */}
      <section className="bg-gray-100 text-gray-800 py-12">
        <motion.div 
          className="text-center mb-12"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-3xl font-bold">Company Timeline</h2>
        </motion.div>

        <div className="max-w-4xl mx-auto">
          <div className="border-l-4 border-gray-400">
            {[
              { year: 2010, title: "Company Founded", description: "Launched with a vision to revolutionize industrial automation" },
              { year: 2015, title: "First Major Innovation", description: "Released our groundbreaking autonomous navigation system" },
              { year: 2018, title: "Global Expansion", description: "Opened offices in Europe and Asia" },
              { year: 2023, title: "Industry Leadership", description: "Recognized as industry leader in robotics innovation" },
            ].map((event, index) => (
              <motion.div 
                key={index} 
                className="mb-8 pl-8"
                initial={{ opacity: 0, x: -20 }}
                whileInView={{ opacity: 1, x: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: index * 0.2 }}
              >
                <div className="text-xl font-bold">{event.year}</div>
                <h3 className="text-lg font-semibold mt-2">{event.title}</h3>
                <p className="text-gray-600 mt-1">{event.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* Our Team Section */}
      <section className="bg-white text-gray-800 py-12">
        <motion.div 
          className="text-center mb-12"
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.6 }}
        >
          <h2 className="text-3xl font-bold">Our Team</h2>
          <p className="text-lg max-w-3xl mx-auto mt-4">
            Meet the innovators and problem solvers behind our success.
          </p>
        </motion.div>

        <div className="flex flex-col md:flex-row justify-center gap-8">
          {[
            { name: "Karthikesh J G", role: "Founder & CEO", image: "https://media.licdn.com/dms/image/v2/D5603AQG3RHTavRa22A/profile-displayphoto-shrink_400_400/profile-displayphoto-shrink_400_400/0/1720688883734?e=1743033600&v=beta&t=J8_DYKGEbEK8mmCSEubHxcxYc97Sf9-NhzgOJhNF0VM" },
            { name: "Raman ", role: "Chief Technology Officer", image: "https://raman-portfolio-007.netlify.app/images/userAsset/image-385x390.jpg" },
            { name: "Elon Musk", role: "Head of Innovation", image: "data:image/webp;base64," },
          ].map((member, index) => (
            <motion.div 
              key={index} 
              className="text-center"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: index * 0.2 }}
            >
              <img className="w-24 h-24 bg-gray-300 rounded-full mx-auto mb-4" src={member.image} alt={member.name} />
              <h3 className="text-xl font-bold">{member.name}</h3>
              <p className="text-gray-600">{member.role}</p>
            </motion.div>
          ))}
        </div>
      </section>
    </div>
  );
};

export default About;
